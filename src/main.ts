...
import {Bishop, Knight, Piece, Queen, Rook} from './class/Piece.ts';
import {setup} from './setup.ts';

// setup the html and the button

const boardContainer: HTMLDivElement =
    document.getElementById('boardContainer') as HTMLDivElement;
const divBoard: HTMLDivElement =
...
const turnIndicator: HTMLHeadingElement =
    document.getElementById('turnIndicator') as HTMLHeadingElement;
const whiteCaptured: HTMLHeadingElement =
    document.getElementById('whiteCaptured') as HTMLHeadingElement;
const blackCaptured: HTMLHeadingElement =
    document.getElementById('blackCaptured') as HTMLHeadingElement;
const moveHistory: HTMLUListElement =
    document.getElementById('moveList') as HTMLUListElement;
  ...
  cells.forEach((row) => {
    row.forEach((cell) => {
      const div = divBoard.querySelector(
                      'div[data-position="' + cell.getPosition() + '"]') as
          HTMLDivElement;
      if (!cell.isCellEmpty()) {
...
function highlightPossibleMoves(): void {
  ...
  possibleMove!.forEach((move) => {
    const div: HTMLDivElement =
        divBoard.querySelector('div[data-position="' + move + '"]') as
        HTMLDivElement;
...
function hidePossibleMoves(): void {
  ...
  cells.forEach((row) => {
    row.forEach((cell) => {
      ...
      div.classList.remove('possible-savior');
    });
  });
}
...
function updateHistoryIndicator(
    bis: boolean = false, ter: boolean = false, check: boolean = true): void {
  if (!bis && !ter) {
...
function updateHistoryIndicatorNormal(): void {
  if (turn === 'white') {
    ...
    li.innerHTML =
        `${turnCount}. <p class="whitemove">${chessBoard.getLastMove()}</p>`;
    moveHistory.appendChild(li);
  } else if (turn === 'black') {
    const lastLi = moveHistory.lastElementChild as HTMLLIElement;
    if (lastLi) {
      lastLi.innerHTML +=
          `<p class="blackmove">${chessBoard.getLastMove()}</p>`;
    }
  }
}
...
function updateHistoryIndicatorTer(check: boolean): void {
  ...
  let li = moveHistory.lastElementChild as HTMLLIElement;
  if (turn === 'white') {
    li.innerHTML = `${turnCount}. <p class="whitemove">${
        chessBoard.getLastMove()}${symbol}</p>`;
    moveHistory.appendChild(li);
  } else if (turn === 'black') {
    li.removeChild(li.lastElementChild!);
    li.innerHTML +=
        `<p class="blackmove">${chessBoard.getLastMove()}${symbol}</p>`;
  }
...
function promotePawn(newPiece: Piece): void {
  ...
  isPawnUpgrading = false;
  let upgradeDiv: HTMLDivElement|null =
      document.getElementById('upgrade-pawn') as HTMLDivElement;
  if (upgradeDiv) {
    upgradeDiv.remove();
  }
...
function upgradePawn(coordinates: {x: number; y: number}, color: string): void {
...
function updateCheck(
  ...
  const wKingDiv =
      divBoard.querySelector(
          'div[data-position="' + wKing.getPosition() + '"]') as HTMLDivElement;
  const bKingDiv =
      divBoard.querySelector(
          'div[data-position="' + bKing.getPosition() + '"]') as HTMLDivElement;
...
function checkForCheck(
    white: boolean, checkArg: boolean, potentielCheckmateArg: boolean,
    kingDiv: HTMLDivElement): void {
  if (checkArg) {
    // king in check
    ...
    kingDiv.classList.remove('check');
}
...
function highlightPossibleSavior(positions: string[]): void {
  positions.forEach((pos) => {
    const div = divBoard.querySelector('div[data-position="' + pos + '"]') as
        HTMLDivElement;
    if (div) {
